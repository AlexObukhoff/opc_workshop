...
/*
  TODO
     ...
     1.  Изменить архитектуру приложения в соответствии с шаблонами источник-манагер-подписчик 
	     основываясь на нижеследующих классах.
	 2.  Для ускорения работы записи/чтения использовать временный дублирующий буфер памяти для 
	     файла дампа в который сейчас производиться запись, записывать на диск и в память отдельно
		 После чего ползать по буферу в памяти читая его. Читать из памяти пока не записана 
