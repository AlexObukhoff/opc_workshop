...
public:
		...
		/*[out, iid_is(riid)]*/ LPUNKNOWN * ppUnk )
	{
		...
		// " аждый пацак должен знать кто здесь чатланин !" (c)
...
//		T* pT = static_cast<T*> (this);
		...
		if( CString(CW2T(szName)) == _T("") )  {
			*ppUnk = NULL; // See OPCDA spec 4.4.4.3
			return E_INVALIDARG;
		}
...
//		T* pT = static_cast<T*> (this);
		...
		if( riid == IID_IEnumUnknown ) 
		{
			...
			if(unkn.size()) {
				unkn.push_back( NULL );
				return  _CreateEnumerator<UnknownEnum,LPUNKNOWN,LPUNKNOWN*>(ppUnk, 
